...
def getPoint(timestamp, reference_point):
    dt = datetime.datetime.strptime(timestamp, "%Y-%m-%d %H:%M:%S")
    return (dt - reference_point).total_seconds()
    

# Calculate time delta and return the row, return in form of delta, row.
def timeDelta(x, reference_point, interest_point):
    point = getPoint(x[0], reference_point)
    delta = point - interest_point
    return (delta, x)
#Calculate time from seconds
def getTimeFromS(seconds):
    str_time = time.strftime('%H:%M:%S', time.gmtime(seconds))
    return datetime.datetime.strptime(str_time,'%H:%M:%S')

def parseDate(date):
    date = date.split(" ")
    return datetime.datetime.strptime(date[0] , '%Y-%m-%d') , datetime.datetime.strptime(date[1] , '%H:%M:%S')

def addTime(hour,added_sec):
    hour = hour.strftime("%H:%M:%S")
    sec = hour.split(":")
    sec = int(sec[0])*3600 + int(sec[1])*60 + int(sec[2])
    sec = added_sec + sec
    return getTimeFromS(sec)


def getSec(date1,date2):
    date1 = date1.strftime("%H:%M:%S")
    sec1 = date1.split(":")
    sec1 = int(sec1[0])*3600 + int(sec1[1])*60 + int(sec1[2])
    
    date2 = date2.strftime("%H:%M:%S")
    sec2 = date2.split(":")
    sec2 = int(sec2[0])*3600 + int(sec2[1])*60 + int(sec2[2])
...
def my_model(ssc, monitoring_dir, current_time, current_stop, seconds_horizon):
    ...
    #Arbitary date point I will refer the deltas to. 
    #In this case getting min numbers. Doesn't matter what we put
    #as far as it is a fixed point in time
    reference_point = datetime.datetime(1, 1, 1)

    # TO BE COMPLETED
    dta = inputDStream.map(lambda line: process_line(line))
    
    #Filer to at stop
    dta = dta.filter(lambda x: (x[9] == 1))
    #Get row and delta
    dta = dta.map(lambda x: timeDelta(x, reference_point, getPoint(current_time, reference_point)))
    #Save state
    dta.persist(pyspark.StorageLevel.MEMORY_AND_DISK)
    #Filter on current stop
    dta1 = dta.filter(lambda x : x[1][8] == current_stop)
    #Filter on time boundary
    dta1 = dta1.filter(lambda x: ((x[0] >= 0) & (x[0] < seconds_horizon)))
    dta1 = dta1.transform(lambda rdd: rdd.sortBy(lambda x: x[0]))
    dta1 = dta1.map(lambda x: (1, x))
    dta1 = dta1.transform(lambda rdd: rdd.reduceByKey(lambda x, y: (x if int(x[0]) < int(y[0]) else y)))
    dta1 = dta1.map(lambda x: (x[1][1][7], x[1][1][0]))
    
    dta2 = dta.filter(lambda x:  ((x[0] >= 0) &  (x[0] < seconds_horizon)))
    dta2 = dta2.map(lambda x: (x[1][7], (x[1][0], x[1][8])))
    #join bus 1 to the rdd
    dta2 = dta2.join(dta1)
    dta2 = dta2.filter(lambda x: x[1][0][0] >= x[1][1])
    dta2 = dta2.mapValues(lambda x: x[0])
    dta2 = dta2.transform(lambda rdd: rdd.groupByKey())
    dta2 = dta2.mapValues(lambda x: list(x))
    ...
    bus_rdd = sorted_dta.transform(lambda rdd: rdd.zipWithIndex()).filter(lambda x : x[1] == 0).map(lambda x :x[0])

    dta = sorted_dta.filter(lambda x : x[1] >= parseDate(current_time)[1] and x[1] <= addTime(parseDate(current_time)[1],seconds_horizon))
    dta.persist(pyspark.StorageLevel.MEMORY_AND_DISK)
    dta = dta.map(lambda x:(x[2],(x[0],x[1],x[3]))).join(bus_rdd.map(lambda x:(x[2],x[0],x[1],x[3])))
    dta = dta.map(lambda x : (x[1][0][0] , x[1][0][1] , x[0] , x[1][0][2]))
    
    dta = dta.map(lambda x : (x[3] , (x[0].strftime("%Y-%m-%d") + " " + x[1].strftime("%H:%M:%S"),getSec(parseDate(current_time)[1] , x[1]))))

    solutionDStream =  dta.reduceByKey(lambda first,second: first + second)
