...
def my_model(spark, monitoring_dir, checkpoint_dir, time_step_interval, bus_stop, bus_line, hours_list):
    ...
    # TO BE COMPLETED
    df = inputSDF.select(inputSDF["date"],inputSDF["busLineID"],inputSDF["closerStopID"],inputSDF["delay"], inputSDF["atStop"])

    df  = df.filter((df["busLineID"] == bus_line) & (df["closerStopID"] == bus_stop) & (df["atStop"] == 1) &\
               (udf_get_hour(df["date"]).isin(hours_list)) &\
               ~(pyspark.sql.functions.dayofweek('date').isin([1, 7])))


    df = df.withColumn("hour" , udf_get_hour(df["date"]))
    #df = df.withColumn("delay" ,  col("delay")))

    df = df.groupby("hour").agg(avg("delay").alias("averageDelay"))
    out = df.withColumn('averageDelay', pyspark.sql.functions.round(col("averageDelay"), 2))

    # Operation O1: We create the DataStreamWritter, to print by console the results in complete mode
    myDSW = out.writeStream\
                       .format("console") \
