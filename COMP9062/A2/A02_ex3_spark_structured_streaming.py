...
udf_get_day = udf(lambda x : getDay(x))

def getHour(date):
   return date[11:13]
  
udf_get_hour = udf(lambda x : getHour(x))
...
def my_model(spark, monitoring_dir, checkpoint_dir, time_step_interval, threshold_percentage):
    ...
    # TO BE COMPLETED
    #Add timestamd for window
    df = inputSDF.withColumn("timestamp", current_timestamp())
    df = df.select("timestamp", "date", "congestion") 
    #Get dat component
    df = df.withColumn("day",udf_get_day(col("date")))
    #Get hour component
    df = df.withColumn("hour",udf_get_hour(col("date")))
    #Drop unneeded date column
    df = df.drop("date")
    #Watermark
    df = df.withWatermark("timestamp", "0 seconds")
    #Group by day and hour
    df = df.groupBy(window("timestamp", my_window_duration_frequency, my_sliding_duration_frequency), col("day"), col("hour"))
    #Aggregate average congesiont
    df = df.agg(avg(col("congestion")))
    #Get columns for the output
    df = df.withColumn("window_start", col("window").start.cast("string"))
    df = df.withColumn("window_end", col("window").end.cast("string"))
    #Drop unneeded
    df = df.drop("window")
    #Round average congestion
    df = df.withColumn("percentage", round(col("avg(congestion)") * 100, 2))
    #Apply threshold filter
    df = df.where(col("percentage") > threshold_percentage)
    #Select columns as per output needed
    solutionSDF = df.select(col("day"), col("hour"), col("window_start"), col("window_end"), col("percentage"))
...
if __name__ == '__main__':
    ...
    my_local_path = "../my_datasets/"
    source_dir = "A02_ex3_micro_dataset/"
    monitoring_dir = "A02_ex3_micro_dataset/my_monitoring/"
    checkpoint_dir = "A02_ex3_micro_dataset/my_checkpoint/"
    ...
    if local_False_databricks_True == False:
        ...
        source_dir = 'FileStore/tables/a2/A02_ex3_micro_dataset/'
