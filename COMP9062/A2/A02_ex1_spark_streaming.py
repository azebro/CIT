...
#Update state as per labs
def updateState(time_interval_list_of_collected_new_values, cur_agg_val):
    # 1. We create the output variable
    res = 0
    ...
    res = cur_agg_val
    if len(time_interval_list_of_collected_new_values) == 1:
      #Add new value to the list, after each iteration the list grows
       res = cur_agg_val + time_interval_list_of_collected_new_values[0]
...
def my_model(ssc, monitoring_dir, bus_stop, bus_line, hours_list):
    ...
    # select the bus line and bus stop
    dta = dta.filter(lambda x : x[1] == bus_line and x[8] == bus_stop and x[9] == 1 and getHour(x[0]) in hours_list)
    # discard weekend days (saturday + sunday)
    dta = dta.filter(lambda x : getWeekDay(x[0]) not in [5,6]) 
    # construct the hour_list
    dta = dta.map(lambda x: (getHour(x[0]), x[6]))
    dta = dta.transform( lambda rdd: rdd.groupByKey())
    dta = dta.mapValues(lambda x: list(x))
    #Update the state, add new value to list
    dta = dta.updateStateByKey(updateState)
    #Calculate rolling avarage (new values are added to list)
    dta = dta.mapValues(lambda x: round(sum(x)/len(x), 2))
    #Sort
    dta = dta.transform(lambda rdd: rdd.sortBy(lambda x: x[-1]))

    dta.persist(pyspark.StorageLevel.MEMORY_AND_DISK)
    
    dta.pprint()

    #solutionDStream = dta.transform(lambda rdd : rdd.sortBy(lambda x : x[1]) )
...
if __name__ == '__main__':
    ...
    if local_False_databricks_True == False:
        ...
        source_dir = 'FileStore/tables/a2/A02_ex1_micro_dataset/'
