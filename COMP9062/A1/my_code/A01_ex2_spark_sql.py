...
def my_main(spark, my_dataset_dir, vehicle_id):
    ...
    # TO BE COMPLETED
    subsetDF = inputDF.filter(inputDF.vehicleID  == vehicle_id)
    subsetDF.persist()


    dayDF = subsetDF.withColumn('day',pyspark.sql.functions.dayofmonth(subsetDF.date)).groupBy("day").agg(pyspark.sql.functions.countDistinct("busLineID").alias('mostBusLines')).sort("mostBusLines", ascending=False).first()
    mostNumberBusLines = dayDF[1]

    dayDF = subsetDF.withColumn('day',pyspark.sql.functions.dayofmonth(subsetDF.date)).groupBy("day").agg(pyspark.sql.functions.countDistinct("busLineID").alias('mostBusLines'), pyspark.sql.functions.sort_array(pyspark.sql.functions.collect_set('busLineID')).alias("sortedBusLineIDs"))
    solutionDF = dayDF.filter(dayDF.mostBusLines == mostNumberBusLines).sort("day").select('day', "sortedBusLineIDs").withColumn("day", pyspark.sql.functions.format_string("%02d", "day"))
