...
def my_main(spark, my_dataset_dir, threshold_percentage):
    ...
    # TO BE COMPLETED
    dayDF = inputDF.withColumn('day',pyspark.sql.functions.dayofmonth(inputDF.date))
    dayhourDF = dayDF.withColumn('hour',pyspark.sql.functions.hour(inputDF.date))

    selectDF = dayhourDF.select('day', 'hour', 'congestion').groupBy('day', 'hour').agg(pyspark.sql.functions.avg('congestion').alias('average'))

    solutionDF = selectDF.select('day', 'hour', 'average' )

    solutionDF = solutionDF.withColumn('percentage',100 * pyspark.sql.functions.col('average') )

    solutionDF = solutionDF.select('day', 'hour', 'percentage').filter(solutionDF.percentage > threshold_percentage)

    solutionDF = solutionDF.sort("percentage", ascending=False).withColumn("day", pyspark.sql.functions.format_string("%02d", "day")).withColumn("hour", pyspark.sql.functions.format_string("%02d", "hour"))

    solutionDF = solutionDF.withColumn("percentage", pyspark.sql.functions.round(solutionDF["percentage"], 2))
