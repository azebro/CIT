...
def my_main(sc, my_dataset_dir, threshold_percentage):
    ...
    # TO BE COMPLETED
    # key will be day, hour and value is congestion
    solutionRDD = inputRDD.map(lambda row: process_line(row)).map(lambda x: ((x[0].split(' ', 1)[0].split('-')[2], x[0].split(' ', 1)[1].split(':')[0]), x[3])).groupByKey().map(lambda x : (x[0], round((float(sum(x[1]))/len(x[1]))*100, 2))).filter(lambda x: x[1] > threshold_percentage).map(lambda x: (x[0][0], x[0][1], x[1])).sortBy(lambda x: x[2], ascending=False)
