...
def my_main(sc, my_dataset_dir, vehicle_id):
    ...
    # TO BE COMPLETED
    subsetRDD = inputRDD.map(lambda row: process_line(row)).filter(lambda row: row[7] == vehicle_id)

    #extract day from the date and aggregate
    filterdayRDD = subsetRDD.map(lambda x: (x[0].split(' ', 1)[0].split('-')[2], x[1])).groupByKey().map(lambda x : (x[0], sorted(list(set(x[1]))), len(list(set(x[1]))))).sortBy(lambda x: x[2], ascending=False)
    filterdayRDD.persist()

    #select only first row and extract mostNumberBusLines
    topdayList = filterdayRDD.take(1)
    mostNumberBusLines = topdayList[0][2]


    solutionRDD = filterdayRDD.map(lambda row: row).filter(lambda row: row[2] == mostNumberBusLines).map(lambda row: (row[0], row[1])).sortBy(lambda x: x[0])
...
if __name__ == '__main__':
    ...
    my_databricks_path = "/"
    my_dataset_dir = "FileStore/tables/a1/"
