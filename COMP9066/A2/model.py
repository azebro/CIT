...
class ChatBotModel:
    ...
    def _create_placeholders(self):
        ...
        self.decoder_inputs = [tf.placeholder(tf.int32, shape=[None], name='decoder{}'.format(i))
                               for i in range(config.BUCKETS[-1][1] + 2)]
    ...
    def _inference(self):
        ...
        single_cell = tf.nn.rnn_cell.BasicLSTMCell(config.HIDDEN_SIZE, state_is_tuple=True)
        single_cell = tf.nn.rnn_cell.DropoutWrapper(single_cell, input_keep_prob=1.0, output_keep_prob=0.5)
        self.cell = tf.nn.rnn_cell.MultiRNNCell([single_cell] * config.NUM_LAYERS, state_is_tuple=True)
    ...
    def _creat_optimizer(self):
        ...
        with tf.variable_scope('training') as scope:
            ...
            if not self.fw_only:
                self.optimizer = tf.train.GradientDescentOptimizer(config.LR)#AdamOptimizer(config.LR)
