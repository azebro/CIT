...
def _check_restore_parameters(sess, saver):
    ...
    ckpt = tf.train.get_checkpoint_state(config.CPT_PATH)
    #ckpt = tf.train.get_checkpoint_state("E:\\checkpoints")
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            if iteration % skip_step == 0:
                ...
                start = time.time()
                total_loss = 0
                saver.save(sess, os.path.join(config.CPT_PATH, 'chatbot'), global_step=model.global_step, write_meta_graph=False, write_state=True)
...
def chat():
    ...
    with tf.Session() as sess:
        ...
        print('Welcome to TensorBro. Say something. Enter to exit and type "feedback:" followed by corrected output, Max length is', max_length)
        ...
        while True:
            ...
            ques.append(line)
            if 'feedback' in line:
                feedbacks.append((ques[-2],ques[-1].split(':')[1]))
                line = _get_user_input()
                #feedback(ques[-2],ques[-1].split(':')[1])
...
def feedback(ques):
    ...
    with tf.Session() as sess:
            ...
            for i in range(len(ques)):
              token_ids_enc.append([data.sentence2id(enc_vocab, str(ques[i][0])),data.sentence2id(enc_vocab, str(ques[i][1]))])
              #token_ids_dec.append(data.sentence2id(enc_vocab, str(ques[i][1])))
            print(np.shape(token_ids_enc))
            #print(token_ids_dec)
            #if len(tokens)>=max_length:
            # tokens = []
            # tokens.extend(token_ids)
            ...
            # Which bucket does it belong to?
            #bucket_id = _find_right_bucket(len(token_ids))
            #print(len(token_ids_enc),max_length)
            ...
            #response = _construct_response(output_logits, inv_dec_vocab)
            #print('corrected answer:',response)
