...
def run(iterations, optimiser, numberOfClasses, dataSize):
    ...
    w = tf.Variable(tf.random.normal([100, numberOfClasses], mean=0.0, stddev=0.05, dtype=tf.float32))
    b = tf.Variable(tf.zeros([1, numberOfClasses]))
    for i in range(iterations):
        with tf.GradientTape() as tape:

            predictions = forward_pass(tr_x, w, b, w1, b1, w2, b2)
            loss = cross_entropy(predictions, tr_y)
            testPredictions = forward_pass(te_x, w, b, w1, b1, w2, b2)
        ...
        gradients = tape.gradient(loss, [w1, b1, w2, b2, w, b])
        optimiser.apply_gradients(zip(gradients, [ w1, b1, w2, b2, w, b]))
